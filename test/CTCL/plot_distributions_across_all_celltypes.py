...
if __name__ == '__main__':
    ...
    p_values_global=pd.read_csv(os.path.join('results', 'p_values_global.csv'))
    ...
    conditions_=sorted(list(np.unique(cell_results.condition)))
    ...
    for heterogeneity_measure in heterogeneity_measures:
        ...
        for j in local_heterogeneity_measure:
            for k in disease_combinations:
                ...
                if len(p_values_global[p_values_global['condition_1']==k[0]][p_values_global['condition_2']==k[1]][p_values_global['score']==j]['p_value_adj'])==0:
                    p_=str(p_values_global[p_values_global['condition_1']==k[1]][p_values_global['condition_2']==k[0]][p_values_global['score']==j]['p_value_adj'].values[0])
                else:
                    p_=str(p_values_global[p_values_global['condition_1']==k[0]][p_values_global['condition_2']==k[1]][p_values_global['score']==j]['p_value_adj'].values[0])
                if float(p_)<0.05:
